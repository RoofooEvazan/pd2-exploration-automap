...
**Unpublished follow-up:** source builds use a hardcoded **33-subtile** radius and add separate boundary/wall color lists, including six new pastel/orange colors. See [color settings](docs/boundary-settings.md). The beta.6 download and existing screenshots show the previous version. Local gameplay confirmation is pending.
