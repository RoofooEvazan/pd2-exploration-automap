...
private:
    struct Entry {Key key{};std::uint64_t epoch=0,coverage=0;bool full=false;std::vector<Rect> rects;};
...
public:
    ...
    template<class Build,class Emit> void query(Key key,Build build,Emit emit) {
        ...
        if(entry.epoch==epoch_ && entry.key==key && (entry.full || entry.coverage==coverage_)){
            ++hits_;for(auto r:entry.rects)emit(r);return;
        }
        ...
        if(scratch_.size()<=rectLimit){
            ...
            entry.rects.assign(scratch_.begin(),scratch_.end());entry.key=key;entry.epoch=epoch_;entry.coverage=coverage_;
