...
The default contour has a 1.0-screen-pixel core (RGB 148,148,148; alpha 224) over a 2.5-pixel dark casing (RGB 24,24,24; alpha 192). Wall Color changes wall cores; recognized water and bank cores use Light Blue (`#50A5DC`, alpha 224). OverlayOpacity scales custom fullscreen alpha. Fractional strokes are clipped across their full width to explored pixels and the viewport. Collision-derived shores retain native water patterns, but not every decorative water surface supplies a floor boundary.
...
The latest 1.13c filename lookup passes automated tests but still needs in-game confirmation on the raised grassy banks. Act 3 bank coloring has been confirmed in offline play. Unrecognized decorative terrain retains its existing rendering.
...
Frame 289 supplies sewer drain/water diamonds. `SewerWater.hpp` cancels shared edges to form a channel perimeter, then adds a slate fill (RGB 86,96,100; alpha 56), dark casing and a core using the selected Water Color (Light Blue by default). Native water patterns stay visible. Bridge frame 290 is excluded. Classification is scoped to `3 Sewer` table rows and levels 92/93 because other areas reuse these IDs; conflicting definitions or object references prevent water styling.
