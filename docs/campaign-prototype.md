...
The default contour uses White (RGB 255,255,255; alpha 224) with a nominal 1.0-screen-pixel core over a 2.5-pixel dark casing (RGB 24,24,24; alpha 192). Wall Color changes wall cores; recognized water and bank cores use Water Color (Light Blue `#50A5DC` by default, alpha 224). Thin strokes snap to a uniform grid that survives D2GL position rounding, with cached geometry to keep walking stable. OverlayOpacity scales custom fullscreen alpha. The full stroke width is clipped to explored pixels and the viewport. Collision-derived shores retain native water patterns, but not every decorative water surface supplies a floor boundary.
