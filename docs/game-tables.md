...
Only three relative game paths are requested: `data/global/excel/Levels.txt`, `automap.txt`, and, when hybrid styling is requested, `Objects.txt`. The reader never loads an extra archive DLL, opens or remounts an MPQ, enables direct access, writes a file, downloads data, or changes native reveal/geometry definitions. No game tables are redistributed.
