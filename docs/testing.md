...
1. Cycle Original, Native, Hybrid and Styled independently under Maps Styling and Campaign Styling, in both views. Check unchanged drawing in Original, original artwork with a boundary in Native, native details in Hybrid, and contour terrain with navigation artwork in Styled. Return to Hybrid and verify retained exploration and colors.
2. Open both color lists in each group. Check mouse/keyboard selection, headings, White, Back/Escape and saved preferences after restarting.
...
4. Cross a town gate and an adjoining campaign boundary, then return. Check the reveal edge immediately on entry, before walking farther. Repeat portal/waypoint travel and hide the map for several seconds. Save/exit and start a new game to check reset behavior.
