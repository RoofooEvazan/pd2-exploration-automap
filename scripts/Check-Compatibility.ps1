...
foreach ($file in $profile.optionalMenuSha256.PSObject.Properties) {
    ...
    else { Write-Host 'Automap settings menu: unrecognized or missing PD2 file; use Style, BoundaryColor and WallColor in the INI Maps/Campaign sections if rows are unavailable.' }
}
if (-not $allMatch) {
