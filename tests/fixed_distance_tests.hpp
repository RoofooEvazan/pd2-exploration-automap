static void testFixedDistance() {
    ...
    DWORD now=500000;
    for(const char* value:{static_cast<const char*>(nullptr),"0","20","31","33","256","999999","-1","invalid"}) {
        ...
        for(const char* mode:{"circle","native-average","invalid"}) {
            ...
            for(auto dims:{std::pair<int,int>{1068,600},{4096,4096},{640,480},{0,INT_MAX}}) {
                ...
                updateForPlayer(p,now+=10);require(maskActive && revealRadius==132);
                Mask reference(.25);reference.revealAround({p.x,p.y},132);
                require(explored->rows()==reference.rows());
                require(explored->contains({p.x+33,p.y}) && !explored->contains({p.x+33.25,p.y}));
                require(!explored->contains({p.x+33,p.y+33}));
            }
        }
    }
    checkContext="fixed circle keeps quarter-cell movement, teleport gaps and town history";
    Mask reference(.25);reference.revealAround({p.x,p.y},132);
    p.x+=.25;updateForPlayer(p,now+=10);reference.revealAround({p.x,p.y},132);
    require(explored->rows()==reference.rows());
    ...
    p.x=1000;updateForPlayer(p,now+=10);reference.revealAround({p.x,p.y},132);
